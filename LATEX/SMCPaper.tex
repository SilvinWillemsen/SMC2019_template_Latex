...
\end{equation}
$\delta(x-x_\text{B})$ is a spatial Dirac delta function selecting the bowing location $x=x_\text{B}$.
...
Approximations for the derivatives in the equations found in Section \ref{sec:PDE} can be found in \cite{Bilbao2009:NumericalSoundSynthesis}. As our implementation uses already existing models, we will not go much into depth in this section, but will rather  We will also use the notation for the difference and averaging operators found here. 
...
\begin{figure}[h]
\centering
\includegraphics[width=1.0\columnwidth]{LATEX/JUCE.pdf}
...
Furthermore, the black arrows indicate instructions that one class can give to another. The hollow arrows show data flows between classes.

All arrows are accompanied by a coloured box indicating on which thread is associated this instruction / dataflow.
