...
\subsection{Plate}
For the plate, the damping coefficients have been decided to be $\sigma_0 = 0.1$ and $\sigma_1 = 0.005$ and the aspect ratio is set to $a = 2$. The plate stiffness $\kappa$ has been left as a user parameter to be changed dynamically and will be between these bounds: $0.1 \leq \kappa \leq 20$. In Equation \eqref{eq:stabilityPlate}, the grid-spacing is calculated using the maximum value of $\kappa$ to prevent stability issues.

\subsection{Connections}
Increasing $\omega_1 \gtrsim 100,000$ while keeping $\omega_0$ relatively low ($0 < \omega_0 \lesssim 100$) will cause non-linear behaviour, such as pitch-glides and rattling sounds. These effects will be more dominant when the plate stiffness is higher.
