...
\end{aligned}
\end{equation}
where $\delta_{x_\text{B}} = \delta(x-x_\text{B})$ is the spatial Dirac delta function (1 at $x_\text{B}$, else 0).
...
\begin{figure*}[t]
    ...
    \caption{A visualisation of Equation \eqref{eq:plateImplementation}. The dots and equations represent the locations $(l,m)$ and what these are multiplied with. (a) An overview. (b) The current time-step $n$. (c) The previous time-step $n-1$. \label{fig:example}}
 \end{figure*}
...
\begin{equation}
    \begin{aligned}\label{eq:stringImplementatino}
        (1 &+ \sigma_0k)u_l^{n+1} = 2u_l^n - (1 - \sigma_0k - 2\phi) u_l^{n-1} \\
        & +(\lambda^2 + \phi)(u_{l+1}^n - 2u_l^n + u_{l-1}^n)\\
        &- \mu^2(u_{l+2}^n - 4u_{l+1}^n + 6u_l^n - 4u_{l-1}^n + u_{l-2}^n) \\
        &+ \phi(u_{l+1}^{n-1} + u_{l-1}^{n-1})\\
        &-k^2\delta_{l_e}F_\text{e},
    \end{aligned}
\end{equation}
where
\begin{equation}\nonumber
\lambda = \frac{\gamma k}{h}, \;\; \mu =  \frac{\kappa k}{h^2}, \;\; \phi = \frac{2\sigma_1k}{h^2} \;\; \text{and} \;\; \delta_{l_\text{e}} = \delta(x-x_{l_\text{e}}). 
\end{equation}
